...
导数描述的是：当输入发生很小的变化时，函数输出大约会变化多快。对于只有一个输入的函数 `f(x)`，可以在 `x` 附近取两个距离相同的点：
...
然后用这两个点之间的平均斜率近似 `x` 处的导数：

```text
f'(x) ≈ [f(x + epsilon) - f(x - epsilon)] / (2 * epsilon)
```

这里的 `epsilon` 是一个很小的正数。分子表示函数从左侧点到右侧点一共变化了多少，分母 `2 * epsilon` 表示这两个输入点之间的实际距离。

这种方法叫“中心差分”，因为目标点 `x` 正好位于两个采样点的中心。与只比较 `f(x + epsilon)` 和 `f(x)` 的前向差分相比，中心差分同时观察目标点左右两侧，通常能够抵消一部分单侧近似误差，因此更适合用来检查自动微分算出的梯度是否合理。
...
如果要近似函数对 `y` 的偏导数，就只能改变 `y`，同时保持 `x` 和 `z` 不变：

```text
右侧输入：f(x, y + epsilon, z)
左侧输入：f(x, y - epsilon, z)
```

`central_difference` 的 `arg` 参数就是用来指出“这次要改变第几个输入”。因此，这个函数计算的不是所有参数的导数，而是针对 `arg` 指定位置的一次偏导数近似。

#### 大概实现逻辑

实现时可以把过程拆成下面几步：

1. 根据原始参数分别准备两组参数，避免直接修改调用者传入的数据。
2. 在第一组参数中，只把索引 `arg` 对应的值增加 `epsilon`。
3. 在第二组参数中，只把同一位置的值减少 `epsilon`。
4. 分别把两组参数传给 `f`，得到目标点右侧和左侧的函数值。
5. 计算两个函数值之差，再除以两个采样点之间的距离 `2 * epsilon`。
6. 返回这个数值近似，供后面的 `derivative_check` 与自动微分结果比较。

这里最重要的约束是：两次调用中只能改变 `arg` 指定的参数，其余参数必须完全相同。`epsilon` 也不是越小越好；过大会让近似不够局部，过小则可能放大浮点数舍入误差。本作业已经提供默认值，实现时按函数签名使用即可。
